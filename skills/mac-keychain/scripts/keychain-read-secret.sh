...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --requesting-actor|--requesting-type|--skill-name|--purpose|--exposure-mode) shift 2 ;;
    --confirmed) shift ;;
...
printf '{"ok":true,"action":"read-secret","service":"%s","account":"%s","keychain":"%s","secret":"%s","secret_exposed_to_user":false}\n' \
